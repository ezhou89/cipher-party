...
- [x] **Step 1: Resolve review findings.** Generate the whole-branch review package against `fe264e3`; review integration contracts, preserved creative sources, release checks, privacy, and any deferred findings. Implement and review concrete fixes before deployment.
- [x] **Step 2: Deploy only after checks pass.** Run the tracked staging deploy with source revision metadata using existing Cloudflare authority. Do not merge/push the original branches. Record Worker version and run `check:staging`; perform a public landing/invite smoke check in desktop/mobile browsers. If a new issue appears, fix it through focused regression and reverify affected gates.
- [x] **Step 3: Update snapshot and handoff.** Record integration branch/worktree, source revisions, exact tests and deploy evidence, archive/design locations, remaining known issues, and the pending four-human session. The integration plan can finish with that human milestone gate explicitly pending; it does not claim Connected Classic or Pack Studio is complete.

#### Step 2 live-smoke regressions discovered September 11

Initial deployment `e780d60` passed authenticated version/binding/transport/hash checks but browser HTML received Cloudflare's injected analytics script, blocked by the intended CSP. Zod's eager object-schema construction also probed `Function`, producing a CSP violation before falling back. Fix these within Step 2; do not relax CSP, filter the errors, change zone-wide settings, or claim the live smoke passed.

- Add `no-transform` to HTML shell responses while retaining `no-store`; leave API/credential caching and JS/CSS policies unchanged. This uses the documented response-local opt-out from Cloudflare body rewriting, not a zone analytics change. Update Worker header regressions and staging attestation, including browser-navigation-like HTML requests and injected-HTML mismatch cases.
- Declare the already-used `zod: ^4.5.4` directly in apps/web (lock retains version 4.5.4; no new external resolution). A first side-effect bootstrap import sets `jitless: true` before any App/router/protocol schema evaluation. Keep the protocol public API and Worker configuration unchanged. This declaration is explicitly approved by the coordinator to avoid an unrelated browser-policy API in the protocol package.
- Add committed production-bundle browser coverage under the actual restrictive CSP, checking `securitypolicyviolation` as well as console/request errors and public landing/invite rendering. It must run offline/local as part of the release gate, never require live rooms or loosen the existing multiplayer privacy harness. Observe the current bundle failing first.
- Run focused RED/GREEN regressions, the complete release gate, and staging dry run; independently review this discovered-defect fix before redeploying. Then repeat explicit live attestation and both public browser smoke cases. The existing namespace and four-human requirement remain unchanged.

#### Completion record — September 11

All four integration tasks are complete. Independent task, whole-branch, and scoped fix reviews have no open findings. Final runtime source `2c727519a69347bd1b914d990f10bb6fa1c5724c` passed the coordinator's full release gate: 533 repository tests, 40 browser cases, eight preflight rows with seven exact expiry identities, documentation/format/lint/types, and builds/dry run.

Tracked staging deployment produced version `9d7bb74d-5eca-4e0a-8b01-da1ebcb206f2`; authenticated live attestation passed source/version/100% traffic, original namespace/bindings, transport/security, and built HTML/JS/CSS hashes. Final read-only desktop/mobile public smoke passed 2/2 with zero application CSP/console/request errors and browser HTML matching the build. Four public screenshots were separately captured and inspected; WebKit's screenshot-helper stylesheet injection is documented separately from application diagnostics, with no relaxed smoke assertions.

Snapshot revision 22 records exact deployment inputs, preserved decisions, creative archive/handoff locations, and next action. Keep the integration branch/worktree and scratch evidence; no merge, push, or apex/zone change. **The four-human session is NOT YET RUN.** Complete `docs/runbooks/connected-classic-playtest.md` before declaring Milestone 1 closed or proceeding to Milestone 2 implementation.
