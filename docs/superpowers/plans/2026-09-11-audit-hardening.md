...
- [x] **Step 1: Add RED gate tests.** Synthetic functions test allowed baseline, cap increase, new over-limit, renamed/stale exception, nested function naming, and excluded files. Release orchestrator injections assert a failed gate prevents deploy, changed HEAD or dirt after gate prevents deploy, expected revision is passed through, and dry-run does not deploy. Verify exact source check remains after build.
...
- [x] **Step 1: Add targeted RED characterization cases.** Open reveal for card A, change nomination to B, then back to A: no dialog resurrection without a fresh click. Open End Turn, change active team/role then cycle back: no stale dialog or send. Compound-invalid clue while guessing/wrong team/invalid count must preserve `wrong_phase`; already-revealed card with no nomination must preserve `already_revealed`, and wrong-team/unknown-card must preserve `wrong_team`. Assert state/action immutability. Existing property/privacy/keyboard tests remain unchanged and passing. If an added characterization already passes, record it honestly; prove it fails by temporarily removing invalid-intent retirement or changing the specific validation precedence rather than claiming RED from a syntax error. Restore mutations before extraction.
- [x] **Step 2: Refactor cohesive logic.** Keep exhaustive action dispatch/default denial and validate current projection at confirmation time. Read React best-practice guidance for hook ownership; do not introduce state-sync effects or client authorization authority.
- [x] **Step 3: Ratchet measured caps down.** Rerun `check:complexity`, update only decreased affected exemptions (remove them once ≤20), report before/after classic and modified scores. Keep permissions/authorization exceptions explicit rather than opportunistically restructuring security code.
- [x] **Step 4: Verify and commit.** Run focused reducer/UI tests, `pnpm run check:release`, and `git diff --check`. Commit `refactor: separate game confirmation and reducer actions`.

Accepted Task 6: `db5b18e71418a948149e15e0af872282730c2e0d`; independent review approved with no findings. Focused reducer/UI tests passed 57/57 and 71/71. Full release gate passed 681 tests, unchanged coverage floors, complexity, builds/preflight, and 48/48 local E2E. Classic complexity fell GameWorkspace 36→10 and applyGameAction 30→18; both exceptions were removed. No deployment/live smoke or human playtest occurred.
...
- [x] **Step 1: Whole-branch review.** Review the complete diff from `7142f5481e72c7e49d127e4877b4e7ee5502df1b`, including task-level deferred findings. Resolve material findings with scoped regression fixes and re-review.
- [x] **Step 2: Fresh final verification.** Run `pnpm run check:release`, `git diff --check`, and `git status --short`; report measured coverage/complexity and any unavailable checks. Do not substitute automated smoke for human acceptance.
- [x] **Step 3: Update handoff.** Record what is fixed, explicitly deferred join/recovery and browser/operational gaps, the pending CI-provider decision, unchanged staging source, and the next real-player session. Keep worktree/branch and evidence; do not merge, publish, or deploy.

Accepted Task 7 / hardening plan: the coordinator reviewed the complete branch from `7142f5481e72c7e49d127e4877b4e7ee5502df1b` through `db5b18e71418a948149e15e0af872282730c2e0d`; all six implementation tasks have distinct clean reviews and no open material findings. Fresh elevated `pnpm run check:release` passed docs/format/lint/types, 681 tests, classic complexity (445 functions, mean 3.38, maximum 34, four explicit exceptions), all five enforced Istanbul floors, builds/local Worker dry run, preflight, and 48/48 local E2E. The sandbox-only listener failure was unavailable in the unprivileged attempt and completed in the approved elevated rerun; no required check remained unavailable. `git diff --check` and `git status --short` were clean. As a subsequent separately authorized operation, the source-bound release deployed `290e8d05d847f2ba80e2625a13222134fe4927ba` to staging; read-only attestation verified 100% version `27df01fb-da1f-4e20-8944-a667dba560a4`, bindings, headers, and three asset hashes, and the artifact-free public smoke passed 4/4. The apex was not changed. Hosted CI still awaits a repository/provider, and the four-human playtest remains NOT YET RUN. Deferred work is explicitly limited to retry-safe lost-join identity/seat recovery and host replacement, browser/screen-reader/real-device and operational drills, and any future pack/mode scope.
